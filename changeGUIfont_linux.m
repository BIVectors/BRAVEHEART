...
% changeGUIfont_linux.m -- Adjust font in GUI on Linux based computers
...
function changeGUIfont_linux(handles)

        for field = fieldnames(handles)'
            field = field{1};
            try 
                set(handles.(field),'FontSize',get(handles.(field),'FontSize')*0.85); % Increase font size by 26% across GUI
            end   
